...
export async function parseBaseTypes() {
  ...
  createReadStream(join(__dirname, '../assets/basetypes.csv'))
    ...
    .on('end', () => {
      ...
      if (!existsSync(join(__dirname, '../src/types/autogenerated'))) {
        mkdirSync(join(__dirname, '../src/types/autogenerated'))
      } else {
        const files = readdirSync(join(__dirname, '../src/types/autogenerated'))
        for (const file of files) {
          unlinkSync(join(__dirname, '../src/types/autogenerated', file))
        }
      }
      ...
      writeFileSync(join(__dirname, '../src/types/autogenerated/ItemClasses.ts'), itemClassesFile)
      ...
      for (const [itemClass, baseTypes] of Object.entries(itemClasses)) {
        ...
        writeFileSync(join(__dirname, `../src/types/autogenerated/${name}.ts`), file)
      }
      ...
      writeFileSync(join(__dirname, '../src/types/autogenerated/index.ts'), indexFile)
    })
}
